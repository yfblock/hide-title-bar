...
export default class NoTitleBarWhenMaximizedExtension extends Extension {
  enable() {
    ...
    console.debug("enable plugin hidetitlebar");
  ...
  _sync(window) {
    ...
    console.debug("window wm class: " +  window.get_wm_class());
    console.warn("window wm class: " +  window.get_wm_class());

    const xWindow = this._xWindow(window);

    let list = ["kitty", "Kitty", "Clash-verge", "clash-verge"];

    if (!xWindow || list.indexOf(window.get_wm_class()) == -1) {
      return;
    }

    console.warn(window.get_wm_class());
